<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Detalles del Lote de Señalamientos para Entrenamiento') }}
        </h2>
    </x-slot>
    ...
    <div>
        <div class="max-w-6xl mx-auto py-10 sm:px-6 lg:px-8">
                            ...
                            <table class="min-w-full divide-y divide-gray-200 w-full">
                                <tr class="border-b">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 bg-white divide-y divide-gray-200">
                                        {{ $lote->id_senalamiento_entrenamiento }}
                                    </td>
                                </tr>
                                <tr class="border-b">
                                    <th scope="col" class="px-6 py-3 bg-purple-900 text-left text-xs font-medium text-white uppercase tracking-wider">
                                            ...
                                            @foreach($imagenes as $index => $ruta)
                                                <div class="relative group">
                                                    ...
                                                    <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black bg-opacity-30">
                                                        <button onclick="mostrarImagenModal('{{ route('senalamientos.entrenamiento.imagen', ['id' => $lote->id_senalamiento_entrenamiento, 'index' => $index]) }}')" 
            ...
            <div class="block mt-8 flex space-x-4">
                <a href="{{ route('senalamientos_entrenamientos.index') }}" class="bg-purple-900 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded">
