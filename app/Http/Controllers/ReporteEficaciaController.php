    ...
    public function downloadExcel(Request $request)
{
    ...
    // Calcular el valor máximo de señales para ajustar la escala del porcentaje
    $maxSenales = 0;
    foreach ($data->labels as $index => $label) {
        $totales = $data->datasets[2]->data[$index];
        if ($totales > $maxSenales) {
            $maxSenales = $totales;
        }
    }
    
    // Factor de escala para los porcentajes (ajustar para que sean visibles)
    $escalaPorcentaje = $maxSenales / 100;
    ...
    foreach ($data->labels as $index => $label) {
        ...
        $sheet->setCellValue('D' . $row, $porcentaje * $escalaPorcentaje); // Ajustar escala para el gráfico
        
        // Mostrar como porcentaje con formato en una columna adicional
        ...
        if ($porcentaje < 50) {
            ...
            $sheet->getStyle('E' . $row)->getFont()->getColor()->setRGB('FFA500');
        } else {
    ...
    // Agregar nota explicativa sobre el porcentaje de eficacia
    $sheet->setCellValue('F' . ($row + 2), 'Nota: Los valores de porcentaje de eficacia han sido escalados para una mejor visualización.');
    $sheet->setCellValue('F' . ($row + 3), 'Los valores reales se muestran en la columna E (0-100%).');
    
    // Pie de página
    $sheet->setCellValue('A' . ($row + 5), 'Generado el: ' . now()->format('d/m/Y H:i:s'));
